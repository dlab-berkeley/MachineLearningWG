...
if (FALSE) {
  ...
  install.packages(c("pvclust", "mclust"))
}
...
Start by using the `hclust` built-in function from [{stats}](https://www.rdocumentation.org/packages/stats/versions/3.5.1). `hclust` prefers a dissimilarity matrix via the `dist` function, thus it plots rows as opposed to columns like the methods below. 
...
plot(as.phylo(hclust_model), type = "fan", tip.color = colors[clus5], lwd = 3, cex = 2)
```
...
Also, see the [cluster](https://cran.r-project.org/web/packages/cluster/cluster.pdf) R package to learn more about agnes, clara, daisy, diana, fanny, flower, mona, and pam cluster methods!  
