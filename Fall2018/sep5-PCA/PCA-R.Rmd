...
1) Load and scale the numeric data (PCA is grossly affected by scale!) and visualize it with boxplots.  
...
# run only lines 46-55 manually to install the packages if necessary
}
...
curves = ggplot(scaled_melt, aes(x = value, fill = Species)) + 
  geom_density(alpha = 0.5) + 
  theme_bw() + 
  facet_wrap(~variable) + theme(legend.position = "top") + 
  scale_fill_manual(values = c("red", "green", "blue")) # custom color scale, just for fun
curves
```
...
```{r}
pca_princomp_plot <- ggplot(pca_princomp_df, 
                            aes(x = pca_princomp_df[,2], y = pca_princomp_df[,3], 
                                color = Species,
                                shape = Species)) + 
...
pca_pc1 <- ggplot(pca_loadings, aes(x = rownames(pca_loadings), y = pca_loadings[,1])) + 
...
# plot for PC 2
pca_pc2 <- ggplot(pca_loadings, aes(x = rownames(pca_loadings), y = pca_loadings[,2])) + 
  geom_bar(stat="identity") +
...
```{r, include = F, echo = F}
pca_loadings_2D <- ggplot(pca_loadings, aes(x = pca_loadings[,1], y = pca_loadings[,2])) + 
...
```{r}
?prcomp
pca_ml <- prcomp(train_x[, -1], retx=TRUE, center=TRUE, scale=TRUE)
pca_ml
