...
#1. What is k-Nearest Neighbor clustering? (kNN)
...
#####5.3. Change "k"
We can also change "k" to evaluate model performance. Ideally you would examine many a variety of kNN k-values and other algorithms simultaneously using the ["SuperLearner" R package](https://cran.r-project.org/web/packages/SuperLearner/index.html), but for today let's just try a few extremes:
```{r}
...
CrossTable(x=test_labels_Z, y=B_p_k2,  # k=2
           prop.chisq=FALSE,
           prop.r=FALSE,
           prop.c=FALSE,
           prop.t=FALSE)
...
library(chemometrics)
#pdf("kNNplot.pdf", 9, 6)
resknn <- knnEval(X, grp, train_plot, knnvec=seq(1,50, by=1), legpo="bottomright", las=2)
title("kNN classification")
#dev.off()
```

###7. Regression example
For the regression, let's return to the B_reg copy that we made at the beginning because it preserved the numeric class of the "dis" variable. 

First, scale the data that now contains the numeric response variable:
```{r}
B_reg <- as.data.frame(scale(B_reg, center=TRUE, scale=TRUE)) # transform the data
```
