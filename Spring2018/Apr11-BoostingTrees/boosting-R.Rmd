...
To improve predictive performance, **bagging**, or bootstrap aggregating will boostrap trees based on 2/3 of observations on all features then average the performance across trees. The out of bag error rate **OOB** is often used to estimate test error of the bagged model and is performed on the remaining 1/3 of observations. Variable importance gets lost in the shuffle!
...
# Install packages
```{r, eval=FALSE}
install.packages(c("car", "caret", "mlbench", "pROC", "randomForest", "rpart", "SuperLearner", "xgboost"))
...
Y_train = ifelse(Y_train == "low", 1, 0) # xgboost only allows numeric input; train response
Y_test = ifelse(Y_test == "low", 1, 0) # xgboost only allows numeric input; test response
...
cv_sl = CV.SuperLearner(X = dat[,-14], 
                        Y = ifelse(dat[,14] == "low",1,0),
                        SL.library = c("SL.xgboost","SL.gbm","SL.rpart","SL.randomForest","SL.mean"),
                        V=5)
